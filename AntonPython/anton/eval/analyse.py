...
def inspect_classification_results_path(base_path, pos): 
    ...
    most_importance_pattern_id = determine_most_importance_col_head(pos, n, [feature_extraction.FeatureCategory.WEB1T_PATTERNS])
...
def determine_most_importance_col_head(pos, n, feature_categories):
    ...
    X, col_heads = learn.feature_matrix(df, [x for x in feature_extraction.FeatureCategory])
    X = StandardScaler().fit_transform(X)
    y = df["rel_class"].values
    ...
    for feature_category in feature_categories:
        requested_col_heads.extend(feature_extraction.make_feature_extractor(feature_category).feature_col_headers())
...
def cluster():
    ...
    X, col_heads = learn.feature_matrix(df, [feature_extraction.FeatureCategory.WEB1T_PATTERNS])
    X = StandardScaler().fit_transform(X)
    y = df["rel_class"].values
...
def compare_pos_category_patterns():
    ...
    def best_ant_feat(pos):
        ...
        best_ant_coeff_feat = list(sorted(zip(clf.coef_[0], col_heads), key=lambda tup: tup[0]))
    ...
    def tex_table(pos_to_best_ant_coeff_feat, pattern_id_to_pattern):

        d = {}
        
        for pos in util.POS:
            
            best_ant_coeff_feat = pos_to_best_ant_coeff_feat[pos]
            best_ant_pattern_coeff = list(map(lambda tup: (pattern_id_to_pattern[tup[1]], tup[0]), best_ant_coeff_feat))

            best_ant_pattern_coeff_sub = best_ant_pattern_coeff[:20]
            best_ant_pattern_coeff_sub.extend(best_ant_pattern_coeff[-10:])

            d["Pattern" + pos.tag] =  [pattern_coeff[0].replace("(\w+)", "X").replace("(\w+)", "Y") for pattern_coeff in best_ant_pattern_coeff_sub]
            d["Coefficient" + pos.tag] = ["{:.2e}".format(pattern_coeff[1]) for pattern_coeff in best_ant_pattern_coeff_sub]
                
        df = pandas.DataFrame(d)
        df = df[["Coefficientadj", "Patternadj", "Coefficientnoun", "Patternnoun", "Coefficientverb", "Patternverb"]]
        
        print(df.to_latex())

    pos_to_best_ant_coeff_feat = {util.POS.ADJ: best_ant_coeff_feat_adj,
                                  util.POS.NOUN: best_ant_coeff_feat_noun,
                                  util.POS.VERB: best_ant_coeff_feat_verb}
    
    print(tex_table(pos_to_best_ant_coeff_feat, pattern_id_to_pattern))

    def intersection(best_ant_feat_1, best_ant_feat_2, n):
        print(len(set(best_ant_feat_1[:n]).intersection(set(best_ant_feat_2[:n]))))
    
    n = 20
     
    intersection(best_ant_feat_adj, best_ant_feat_noun, n)
    intersection(best_ant_feat_adj, best_ant_feat_verb, n)
    intersection(best_ant_feat_noun, best_ant_feat_verb, n)
...
def one_classifier():
    ...
    X_adj, y_adj = load_X_y(util.POS.ADJ)
    X_noun, y_noun = load_X_y(util.POS.NOUN)
    X_verb, y_verb = load_X_y(util.POS.VERB)
    
    X = np.concatenate((X_adj, X_noun, X_verb), axis=0)
    y = np.concatenate((y_adj, y_noun, y_verb), axis=0)
    
    clf = RandomForestClassifier()
    
    scores = cross_validation.cross_val_score(clf, X, y, cv=10, scoring='f1_weighted', n_jobs=-1)
    print("Cross-validation for pos combined: " + str(scores.mean()))
...
if __name__ == "__main__":
    ...
    # print_rf_most_important_features([x for x in feature_extraction.FeatureCategory], util.POS.NOUN, 50)
    # print_rf_most_important_features([feature_extraction.FeatureCategory.WEB1T_PATTERNS], util.POS.VERB, 50)
    ...
    classification_examples("verb_ants_all", util.POS.VERB)
